...
def run_cmd(app, cmd, errs=False, norm=True):
    """Clear out and err StdSim buffers, run the command, and return stdout if errs=False or stderr if errs=True"""
...
class test_PySh(unittest.TestCase):
    ...
    def test_zzzfind(self):
        ...
        out = run_cmd(PApp, "find /sympy/interactive -iname *version* -winner='-p.depth' --depthfirst", norm=True)
        # get Leader updates from stderr (since quiet is false)
        out2 = run_cmd(PApp, 'find /sympy/interactive -iname *version* -winner="-p.depth" --depthfirst', norm=True, errs=True)
        # check results
        ...
        out = run_cmd(PApp, "find /sympy/interactive -iname *version* -winner='-p.depth' --depthfirst", norm=True, errs=True)
        self.assertEqual(len(out), 0)


        # check that limit 1 works, that and wildcard name matching works

        out=run_cmd(PApp, "find . -name trun* --depthfirst -limit 1 -maxdepth 5", norm=True)
        self.assertEqual(out, ['/sympy/Poly/trunc'])

        # check that find -a searches within hidden namepaces
        out = run_cmd(PApp, "find -a -maxdepth 3 /sympy/And -name *r*p* --depthfirst -limit 5", norm=True)
        ...
        out = run_cmd(PApp, 'find /sympy -name sin --depthfirst -print "p.abspath" -limit 4', norm=True)
        self.assertEqual(len(out), 4)

        # out = run_cmd(PApp, 'find /sympy -name sin --depthfirst -print p.abspath', norm=True)  # non-determistic for dynamic attributes
        out = run_cmd(PApp, 'find /sympy -maxdepth 5 -name sin -print p.abspath', norm=True)
...
if __name__ == '__main__':
    # pob.shell()
    PApp = pob.Pobiverse(,,,
    # run_cmd(PApp, 'set static false')  WIP
